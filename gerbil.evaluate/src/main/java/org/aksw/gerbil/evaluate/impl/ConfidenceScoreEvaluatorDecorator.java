...
public class ConfidenceScoreEvaluatorDecorator<T extends Model> extends AbstractEvaluatorDecorator<T> {
	...
	private String confidenceURI = "http://ontology.thomsonreuters.com/supplyChain#aggregatedConfidenceScore";
	...
	public ConfidenceScoreEvaluatorDecorator(Evaluator<T> evaluator, String resultName,
		...
		this.confidenceURI = confidenceURI;
	...
	public void evaluate(List<List<T>> annotatorResults, List<List<T>> goldStandard,
		...
		EvaluationResultContainer bestResult = evaluate(annotatorResults, goldStandard, results, 0, scores[scores.length-1]);
	...
	protected double[] getConfidenceScores(Model annotatorResults) {
		DoubleOpenHashSet scores = new DoubleOpenHashSet();
		boolean foundMarkingWithoutScore = false;
		Set<Resource> subjects = annotatorResults.listSubjects().toSet();
		for (Resource subject : subjects) {
			Statement confidenceStmt = subject.getProperty(this.confidenceProperty);
			if (confidenceStmt != null) {
				scores.add(confidenceStmt.getDouble());
			} else {
	...
	protected EvaluationResultContainer evaluate(List<List<T>> annotatorResults, List<List<T>> goldStandard,
			EvaluationResultContainer results, double threshold, double max) {
		EvaluationResultContainer currentResults = new EvaluationResultContainer(results);
		//create filter!
		ModelFilter<T> filter = new ConfidenceScoreBasedModelFilter<T>(max, threshold, confidenceProperty) ;
		this.evaluator.evaluate(filter.filter2ListOfLists(annotatorResults.get(0).get(0)), goldStandard, currentResults);
