...
public class DBpediaSpotlightClient {
    ...
    protected List<NamedEntity> parseResponse(String response) {
        if (response == null) {
            return null;
        }
        @SuppressWarnings("unchecked")
        List<NamedEntity> annotations = gson.fromJson(response, List.class);
        return annotations;
    }

    public List<NamedEntity> annotate(String text) {
        if (text == null) {
            return null;
        }
        text = text.trim();
    ...
    private static class AnnotationsDeserializer implements JsonDeserializer<List<NamedEntity>> {
        ...
        public List<NamedEntity> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
                throws JsonParseException {
            JsonObject responseAsObject = json.getAsJsonObject();
            List<NamedEntity> annotations = new ArrayList<NamedEntity>();
            if (responseAsObject.has(NAMED_ENTITIES_ARRAY_KEY)) {
                ...
                for (JsonElement resource : resources) {
                    ...
                    if (annotation.has(NAMED_ENTITY_URI_KEY) && annotation.has(NAMED_ENTITY_OFFSET_KEY)
                        ...
                        annotations.add(new NamedEntity(annotation.get(NAMED_ENTITY_OFFSET_KEY).getAsInt(), annotation
                                .get(NAMED_ENTITY_SURFACE_FORM_KEY).getAsString().length(), uri));
                    }
                }
            }
            return annotations;
