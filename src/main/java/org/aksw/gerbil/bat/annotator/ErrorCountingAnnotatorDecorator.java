...
public class ErrorCountingAnnotatorDecorator {
	...
	protected static HashSet<Annotation> solveD2W(
		...
		try {
			return ((D2WSystem) errorCounter.getDecoratedAnnotator()).solveD2W(
					text, mentions);
		} catch (Exception e) {
			if (errorCounter.getErrorCount() == 0) {
				// Log only the first exception completely
	...
	protected static HashSet<Annotation> solveA2W(
			AbstractErrorCounter errorCounter, String text) {
		try {
			return ((A2WSystem) errorCounter.getDecoratedAnnotator())
					.solveA2W(text);
		} catch (Exception e) {
			if (errorCounter.getErrorCount() == 0) {
				// Log only the first exception completely
	...
	protected static HashSet<ScoredTag> solveSc2W(
			AbstractErrorCounter errorCounter, String text) {
		try {
			return ((Sc2WSystem) errorCounter.getDecoratedAnnotator())
					.solveSc2W(text);
		} catch (Exception e) {
			if (errorCounter.getErrorCount() == 0) {
				// Log only the first exception completely
	...
	protected static HashSet<ScoredAnnotation> solveSa2W(
			...
			throws AnnotationException {
		try {
			return ((Sa2WSystem) errorCounter.getDecoratedAnnotator())
					.solveSa2W(text);
		} catch (Exception e) {
			if (errorCounter.getErrorCount() == 0) {
				// Log only the first exception completely
