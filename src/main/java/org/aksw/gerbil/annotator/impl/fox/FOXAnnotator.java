...
public class FOXAnnotator implements OKETask1Annotator {
    ...
    protected Document requestAnnotations(Document document)
	    throws GerbilException {
	...
	try {
	    ...
	    Response response = Request
		    ...
		    .body(new StringEntity(new JSONObject()
			    .put("input", document.getText())
			    .put("type", "text").put("task", "ner")
			    .put("output", "JSON-LD").toString(),
	    ...
	    throw new GerbilException(
		    "Got an exception while communicating with the FOX web service.",
		    e, ErrorTypes.UNEXPECTED_EXCEPTION);
	}
	return resultDoc;
    }

    protected void parseType(JSONObject entity, Document resultDoc)
	    throws GerbilException {
	try {

	    if (entity != null && entity.has("means")
		    && entity.has("beginIndex") && entity.has("ann:body")) {
		...
		if (begin instanceof JSONArray) {
			...
			resultDoc.addMarking(new TypedNamedEntity(Integer
				.valueOf(((JSONArray) begin).getString(i)),
				body.length(), uri, types));
		    }
		} else if (begin instanceof String) {
		    resultDoc
			    .addMarking(new TypedNamedEntity(Integer
				    .valueOf((String) begin), body.length(),
				    uri, types));
	    ...
	    throw new GerbilException(
		    "Got an Exception while parsing the response of FOX.", e,
	...
	types.add(typeString.replaceFirst("scmsann:",
