...
public abstract class AbstractNIFDocumentParser implements NIFDocumentParser {
	...
	protected abstract Model parseNIFModelFromReader(Reader reader);
	...
	protected AnnotatedDocument createAnnotatedDocument(Model nifModel)
		...
		resultDocument.setDocumentURI(document.getURI());
		...
		while (resIter.hasNext()) {
			...
			if ((start >= 0) && (end >= 0)) {
				nodeIter = nifModel.listObjectsOfProperty(annotationResource,
						ITSRDF.taIdentRef);
				if (nodeIter.hasNext()) {
					entityUri = nodeIter.next().asLiteral().getString();
				...
				// FIXME how to handle the problem, that if the annotator
				// returns mutliple disambiguations for a single annotation the
				// NIF is not really able to handle this. The only way is to
				// create a single annotation node for every
				// disambiguation-confidence pair.
			} else {
