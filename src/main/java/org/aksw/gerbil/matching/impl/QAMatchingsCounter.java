...
public class QAMatchingsCounter implements MatchingsCounter<AnswerSet> {
    ...
    public QAMatchingsCounter(TripleIndex index, UrlValidator urlValidator, Literal2ResourceManager converterManager) {
        this(index, urlValidator, null, null, converterManager);
    }
    ...
    public EvaluationCounts countMatchings(List<AnswerSet> annotatorResult, List<AnswerSet> goldStandard) {
        ...
        for (Object answerGS : goldStdAnswerSet.getAnswers()) {
            ...
            if (answerGS instanceof Annotation) {
                ...
                if (answerGS != null) {
                    ...
                    // if there is no value in an answer, it is invalid format, however we will
                    // assume that the answer is just empty as default.
                    // FIXME maybe we should throw a detailed error here, so the user knows what's
                    // wrong.
                    answer = "";
                }
         ...
         *
         * as we can assume that if a GS is a string the annotator either provides a
         * string or a false annotation. The remaining annotator results can then be
         * forced to annotations and be added to the actual annotation results of the
         * system
         */
        ...
        // Hot fix: the comparison of Strings with a meaning matcher does not
        // make sense, since it will treat them as emerging entities. Hence, we have to
        ...
        /*
         * all which are not found by the system will be viewed in the anno comp, as the
         * string comp will guide the fp through and the annotation comp will then check
         * if they are really fp or if they were an annotation in the other set
         */
        global.setFalseNegatives(annotationCounts.falseNegatives + stringCounts.falseNegatives);
