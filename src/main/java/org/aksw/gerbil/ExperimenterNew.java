...
import org.aksw.gerbil.execute.ExperimentTaskNew;
...
public class ExperimenterNew implements Runnable {

    private static final Logger LOGGER = LoggerFactory.getLogger(ExperimenterNew.class);
     ...
     */
    @Deprecated
    public ExperimenterNew(Overseer overseer, ExperimentDAO experimentDAO, ExperimentTaskConfiguration configs[],
            String experimentId) {
    ...
    public ExperimenterNew(Overseer overseer, ExperimentDAO experimentDAO, SameAsRetriever globalRetriever, EvaluatorFactory evFactory,
        ...
        try {
            ...
            for (int i = 0; i < configs.length; ++i) {
                if (couldHaveCachedResult(configs[i])) {
                    taskId = experimentDAO.connectCachedResultOrCreateTaskNew(configs[i].annotatorConfig.getName(),
                            configs[i].datasetConfig.getName(), configs[i].type.name(), configs[i].matching.name(),
                            experimentId);
                } else {
                    taskId = experimentDAO.createTaskNew(configs[i].annotatorConfig.getName(),
                            configs[i].datasetConfig.getName(), configs[i].type.name(), configs[i].matching.name(),
                            experimentId);
                }
                ...
                if (taskId != ExperimentDAO.CACHED_EXPERIMENT_TASK_CAN_BE_USED) {
                    ...
                    ExperimentTaskNew task = new ExperimentTaskNew(taskId, experimentDAO, globalRetriever, evFactory,
                            configs[i]);
